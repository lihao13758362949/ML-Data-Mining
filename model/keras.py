...
# model.summary() model总结，看看model建成什么样了

from keras.layers import Input # 输入层
from keras.layers import Dense # 全连接层
# Dense(units, activation=None, use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, kernel_constraint=None, bias_constraint=None)
# units：输出神经元个数。
from keras.layers import Activation # 激活函数层，你可以选择在其他层加入activation参数，也可以调用Activation层来指定激活函数,relu 或 softmax：多类别sigmid 或 sigmoid：二分类
# relu(x, alpha=0.0, max_value=None, threshold=0.0)
# tanh(x)
# sigmoid(x)
# linear(x)：缺省时采用这个。
# softmax(x, axis=-1)
from keras.layers.core import Reshape # 转换shape
# Reshape((3, 4), input_shape=(12,)) 
from keras.layers import concatenate, Add, ZeroPadding2D, BatchNormalization, 
